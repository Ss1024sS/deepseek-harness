...
`inject: ['agents']`。服务器按 `sessionId` 创建或显式恢复一个 agent。只有服务对生命周期建立快照时记录的 `local` 标志为 true，服务器才会转发 subagent 完成事件；提供方名称、子级 id 和持久化谱系均不能证明本地性。已注册的适配器优先；尚无适配器负责的 `deepseek-official` 路由会挂载 `dsh-llm-deepseek`，任何其他尚无适配器负责的提供方都会导致初始化失败。其他能力由外围 `cordis.yml` 提供。
...
`initialize.serverInfo.name` 的协议稳定值为 `deepseek-harness-sdk-runtime`。每个服务器只接受一次成功的 `initialize`：此前的 `session/prompt` 和 `session/resume` 会拒绝，并发或后续初始化也会拒绝，而失败的初始化可以在不提交路由的情况下重试。可选的正整数 `initialize.maxTokens` 会成为每个 SDK 创建或恢复的 agent 及其进程内后代的请求输出上限；非法值会使初始化失败，省略时则不发送 SDK 上限，并应用所选适配器或提供方路由的默认值。`session/resume` 会在初始化时选定的提供方、模型、token 上限和精确 cwd 下显式恢复一个持久会话；它必须先于该进程对该会话的第一次 `session/prompt`，并返回 `{ sessionId, durablePrefixCount, nextSeq }`，其中两个游标字段位于恢复 seed 标记的两侧。会话不存在、cwd 不匹配、会话重复、已打开或已由 prompt 创建，以及服务器正在关闭时都会拒绝。`session/prompt` 将一条带标识的用户消息排入队列，并立即返回 `{ messageId }`。服务器将每个持久事实作为 `session.event` 流式发出，并将整个 agent 生命周期的每次状态转换作为 `session.status` 发出；它不会把某条助手消息或 `turn/end` 归属于该提示词。同一会话上的独立请求可以继续排入更多工作。持久化根目录和 persona 由 `cordis.yml` 提供。
